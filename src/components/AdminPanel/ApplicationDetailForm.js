  ...
  const ApplicationDetailForm = ({detail,setDetail,handleDetailChange,fieldSettings, canRead = false, canWrite = false }) => {
    ...
    const fetchApplications = async () => {
      try {
        ...
        const response = await getApplicationUserId(userId);
        ...
        if (data) {
          setDetail({
            ...
            monthlyIncome: data.monthlyGrossIncome || "",
            ssn: data.ssn || "",
            confirmSsn: data.ssn || "",
            amountNeeded: data.howMuchDoYouNeed || "",
            homeAddress: data.homeAddress || "",
            homeAddress2: data.homeAddress2 || "",
            zipCode: data.zipCode || "",
            city: data.city || "",
            state: data.state || "",
            homeowner: data.isHomeOwner ? "yes" : "no",
            agreeTerms: data.agreeTerms || false,
            authorizeCredit: data.authorizeCredit || false,
          });
        }
      } catch (error) {
    ...
    if (validate()) {
      try {
        ...
        const requestData = {
          ...
          state: detail.state,
          isHomeOwner: detail.homeowner === "yes",
          createdBy: sessionStorage.getItem("role").toLowerCase(), // 👈 Only include in edit
        };
